...
struct  PERSON 
	...
	PERSON * readData(int N);  // readData prototype <NEW>
...
int main()
{
	...
	PERSON * arrPtr = readData(N); // create a holder pointer variable arrPtr to store the readData value. 
			
	int choice;
	bool programOn = true;


while (programOn != false)   // switch statement for the output menu. 
{
cout << "\n";
cout << "*******************************\n";
...
delete [] arrPtr;  // deletes the dynamic array. <NEW>
return 0;
}
...
PERSON * readData(int N)  // readData function returns a new Pointer array.  <NEW>
{
